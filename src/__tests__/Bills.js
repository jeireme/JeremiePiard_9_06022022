...
describe("Given I am connected as an employee", () => {
  describe("When I am on Bills Page", () => {
    /*test("Then the number of bills should be positive", () => {
      ...
      const billsJs = new Bills({document, onNavigate, store:null, bills:bills, localStorage: window.localStorage})
      
      document.body.innerHTML = BillsUI({ data: bills });
      console.log(billsJs);
      console.log(billsJs.getBills()); // ? undefined
      expect(billsJs.getBills).toBeGreaterThanOrEqual(0);
    });*/

    test('When I click on the button "Nouvelle note de frais", it should open call handleClickNewBill()', async () => {
     
      // ? test pour accéder à la methode handleClickNewBill()
      const onNavigate = (pathname) => {
        document.body.innerHTML = ROUTES({ pathname });
      };
